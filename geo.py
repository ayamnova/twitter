...
The most important method is get_geo which will find a distribution of 
sentiment across a geographic region and can filter them by country (default) 
or by state (only within the United State).
...
# A dictionary of GPS coordinates retrieved originally from a Google list
# and modified
COUNTRY_COORD = {
...
'''The following function was consolidated from anther module. Still needs to be
...
'''
The following functions are from another module that was consolidated with
this one. Still need to verify this code
'''
...
'''
The following functions are related to getting a sentiment distribution across
a region.
'''
